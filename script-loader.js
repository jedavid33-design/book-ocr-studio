// Book OCR Studio 2.2 loader. Keeps the review-toggle compatibility patch.
// Keeps the existing script.js intact, injects the two missing click handlers,
// then runs the patched source.

(async () => {
  const response = await fetch("./script.js?v=2.2.0", { cache: "no-store" });
  if (!response.ok) {
  ...
  source += "\n//# sourceURL=book-ocr-studio-2.2.0.js";
