// Book OCR Studio 154 loader. Keeps the review-toggle compatibility patch.
// Keeps the existing script.js intact, injects the two missing click handlers,
// then runs the patched source.

(async () => {
  const response = await fetch("./script.js?v=154", { cache: "no-store" });
  if (!response.ok) {
  ...
  source += "\n//# sourceURL=book-ocr-studio-154.js";
