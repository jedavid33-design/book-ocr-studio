...
(async () => {
  ...
  async function exportLabeledWordCrops(){
    ...
    console.log("Classifier crop export: entering exporter", {pages:state.pages.length,files:state.files.length});
    setStatus("Classifier crop export · loading learning profile…");
    await new Promise(resolve=>setTimeout(resolve,0));
    const profile=currentItalicLearningProfile();
    console.log("Classifier crop export: profile loaded", {examples:(profile.examples||[]).length});
    const eligible=(profile.examples||[]).filter(x=>(x.label==="ITALIC"||x.label==="ROMAN")&&!x.fragment&&romanResidualExampleKey(x));
    const italics=eligible.filter(x=>x.label==="ITALIC");
    const romans=eligible.filter(x=>x.label==="ROMAN");
    console.log("Classifier crop export: eligible labels", {eligible:eligible.length,italics:italics.length,romans:romans.length});
    setStatus("Classifier crop export · "+italics.length+" Italic · "+romans.length+" Roman labels found");
    await new Promise(resolve=>setTimeout(resolve,0));
    if(!italics.length||!romans.length){setStatus("Need persisted Roman and Italic labels before exporting word crops.");return;}
    ...
    const cleanSingle=x=>{
      const text=String(x.normalizedText||x.specimenText||"").trim();
      const words=text.split(/\s+/).filter(Boolean);
      const span=(Number.isFinite(Number(x.startWordIndex))&&Number.isFinite(Number(x.endWordIndex)))?Math.abs(Number(x.endWordIndex)-Number(x.startWordIndex))+1:words.length;
      return !x.fragment&&words.length===1&&span===1;
    };
    const cleanItalics=italics.filter(cleanSingle);
    const cleanRomans=romans.filter(cleanSingle);
    const rankedRomans=cleanRomans.map((x,i)=>{
    ...
    const hardCount=Math.min(1000,rankedRomans.length);
    const hardRomanSample=rankedRomans.slice(0,hardCount).map(r=>r.x);
    const hardKeys=new Set(hardRomanSample.map(romanResidualExampleKey));
    const ordinaryPool=cleanRomans.filter(x=>!hardKeys.has(romanResidualExampleKey(x)));
    const ordinaryRomanSample=spreadSample(ordinaryPool,Math.min(1000,ordinaryPool.length));
    const selected=[...cleanItalics,...hardRomanSample,...ordinaryRomanSample];
    console.log("Classifier crop export: training corpus selected",{cleanItalics:cleanItalics.length,hardRomans:hardRomanSample.length,ordinaryRomans:ordinaryRomanSample.length,total:selected.length});
    const byKey=new Map();
    for(const ex of selected){const key=romanResidualExampleKey(ex);if(key&&!byKey.has(key))byKey.set(key,ex);}
    const manifest=[],files=[];
    let encoded=0;
    console.log("Classifier crop export: key map ready", {keys:byKey.size});
    setStatus("Classifier crop export · "+byKey.size+" sampled labels · matching pages…");
    await new Promise(resolve=>setTimeout(resolve,0));
    for(let pageIndex=0;pageIndex<state.pages.length;pageIndex++){
      ...
      console.log("Classifier crop export: first matching page", {pageIndex:pageIndex,wanted:wanted.length});
      setStatus("Classifier crop export · "+encoded+"/"+byKey.size+" crops · page "+(pageIndex+1)+"/"+state.pages.length);
      await new Promise(resolve=>setTimeout(resolve,0));
      let img;
      try{
        ...
        console.warn("Classifier crop export: skipping page",pageIndex,"("+String(err?.message||err)+")");
        setStatus("Classifier crop export · skipped page "+(pageIndex+1)+" · continuing…");
        await new Promise(resolve=>setTimeout(resolve,16));
        continue;
      }
      console.log("Classifier crop export: image loaded",{pageIndex:pageIndex});
      ...
      for(const [key,ex] of wanted){
        ...
        const evidence=state.italicValidationEvidenceByPhysical instanceof Map?state.italicValidationEvidenceByPhysical.get(key):null;
        const candidates=[ex.reviewBox,ex.box,evidence?.reviewBox,word?.box].filter(Boolean);
        let box=candidates.find(b=>Number(b.w??b.width??0)>1&&Number(b.h??b.height??0)>1);
        if(!box&&Array.isArray(line?.italicWordMeta)){
          const text=String(ex.normalizedText||ex.specimenText||"").trim().toLocaleLowerCase();
          const match=line.italicWordMeta.find(w=>String(w?.text||"").trim().toLocaleLowerCase()===text&&Number(w?.box?.w||0)>1&&Number(w?.box?.h||0)>1);
          box=match?.box||null;
        }
        if(!box&&line?.box){
          try{
            const rebuilt=estimateInkAlignedWordBoxes(canvas,line)||[];
            const text=String(ex.normalizedText||ex.specimenText||"").trim().toLocaleLowerCase();
            const indexed=rebuilt[parts[2]];
            if(indexed?.box&&Number(indexed.box.w||0)>1&&Number(indexed.box.h||0)>1)box=indexed.box;
            if(!box&&text){
              const match=rebuilt.find(w=>String(w?.text||"").trim().toLocaleLowerCase()===text&&Number(w?.box?.w||0)>1&&Number(w?.box?.h||0)>1);
              box=match?.box||null;
            }
            if(box)console.log("Classifier crop export: rebuilt OCR word box",key,{box:box});
          }catch(err){console.warn("Classifier crop export: word-box rebuild failed",key,err);}
        }
        if(!box){console.warn("Classifier crop export: no usable box",key,{exampleBox:ex.reviewBox||ex.box||null,wordBox:word?.box||null,evidenceBox:evidence?.reviewBox||null,lineBox:line?.box||null});continue;}
        const bw=Number(box.w??box.width??0),bh=Number(box.h??box.height??0);
        const bx=Number(box.x??box.left??0),by=Number(box.y??box.top??0);
        const pad=Math.max(2,Math.round(bh*.18));
        const x=Math.max(0,Math.floor(bx-pad)),y=Math.max(0,Math.floor(by-pad));
        const w=Math.min(canvas.width-x,Math.ceil(bw+pad*2)),h=Math.min(canvas.height-y,Math.ceil(bh+pad*2));
        if(w<2||h<2){console.warn("Classifier crop export: unusable crop geometry",key,{box:box,canvas:[canvas.width,canvas.height],crop:[x,y,w,h]});continue;}
        ...
        console.log("Classifier crop export: encoding crop",{pageIndex:pageIndex,lineIndex:parts[1],wordIndex:parts[2],w:w,h:h});
        let blob;
        try{
          blob=await Promise.race([
            new Promise(resolve=>crop.toBlob(resolve,"image/png")),
            new Promise((_,reject)=>setTimeout(()=>reject(new Error("crop PNG timeout")),5000))
          ]);
        }catch(err){
          console.warn("Classifier crop export: skipping crop",key,"("+String(err?.message||err)+")");
          crop.width=1;crop.height=1;
          continue;
        }
        console.log("Classifier crop export: crop encoded",{pageIndex:pageIndex,lineIndex:parts[1],wordIndex:parts[2],bytes:blob?.size||0});
        crop.width=1;crop.height=1;
        if(!blob)continue;
        const name=(ex.label==="ITALIC"?"italic":"roman")+"/"+String(manifest.length+1).padStart(4,"0")+"_"+pageIndex+"_"+parts[1]+"_"+parts[2]+".png";
        files.push({name,blob});
        manifest.push({file:name,label:ex.label,text:String(ex.normalizedText||ex.text||ex.specimenText||word?.text||""),pageIndex,lineIndex:parts[1],wordIndex:parts[2],sourceFile:file.name,box:{x,y,w,h},specimenId:String(ex.id||""),sampleRole:ex.label==="ITALIC"?"italic":(hardKeys.has(key)?"hard-roman":"ordinary-roman"),existingVector:Array.isArray(ex.vector)?ex.vector:null});
        encoded++;
        if(encoded%5===0){
          setStatus("Classifier crop export · "+encoded+"/"+byKey.size+" crops");
          await new Promise(resolve=>setTimeout(resolve,16));
        }
      }
    ...
    console.log("Classifier crop export: traversal complete",{encoded:encoded,files:files.length,manifest:manifest.length});
    if(!files.length){setStatus("No labeled word crops could be matched to the recovered screenshots.");return;}
    console.log("Classifier crop export: checking JSZip",{available:typeof JSZip!=="undefined"});
    if(typeof JSZip==="undefined")throw new Error("JSZip is required for crop export.");
    setStatus("Classifier crop export · packaging "+files.length+" crops…");
    await new Promise(resolve=>setTimeout(resolve,16));
    const zip=new JSZip();
    ...
    const cropManifest={format:"book-ocr-studio-labeled-word-crops-v2",buildVersion:BUILD_VERSION,exportedAt:new Date().toISOString(),selection:"all clean single-word confirmed Italics + up to 1000 structurally hard confirmed Romans + up to 1000 deterministic ordinary confirmed Romans",count:manifest.length,italic:italicCount,roman:romanCount,source:"original screenshot crop",items:manifest};
    ...
    downloadBlob(out,"italic-visual-training-corpus-v180.zip");
  ...
  function runRomanResidualExperiment(){
    ...
    downloadBlob(new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),"italic-roman-residual-v180.json");
  ...
  function analyzeRomanResidualExport(payload){
    ...
    downloadBlob(new Blob([JSON.stringify(result,null,2)],{type:"application/json"}),"italic-roman-residual-penalty-v180.json");
...
`;
  ...
  source += "\n//# sourceURL=book-ocr-studio-180.js";
  (0, eval)(source);
})().catch((err) => {
