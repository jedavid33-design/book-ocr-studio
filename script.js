(() => {
  ...
  async function exportTypographyTest() {
    ...
    try {
      ...
      const chatgptQaInstructions = `# BOOK OCR STUDIO — PIXEL-BY-PIXEL QA INSTRUCTIONS
...
`;
      ...
      const qaRouterInstructions = "# BOOK OCR STUDIO — QA INSTRUCTION ROUTER\n\nThis Typography Test ZIP includes separate engine-specific QA instructions.\n\n- If you are ChatGPT, read and follow CHATGPT-QA-INSTRUCTIONS.md.\n- If you are Meta Muse/Wren, read and follow MUSE-QA-INSTRUCTIONS.md.\n- If you are another QA engine, use CHATGPT-QA-INSTRUCTIONS.md as the canonical baseline unless the user tells you otherwise.\n\nBoth workflows use the same source of truth: the original screenshots in images/ and the stable IDs in typography-map.json.\n\nIMPORTANT: Do not invent a new JSON schema. The only importer-ready cumulative QA format is:\nbook-ocr-studio-cumulative-chapter-qa-checkpoint-v1\n\nBook-specific notes supplied by the user in chat override neither the screenshot pixels nor the canonical importer schema; they only add context such as expected chapter count, POV labels, chapter titles, or special layout conventions.\n";
      const museQaInstructions = "# BOOK OCR STUDIO — META MUSE / WREN WHOLE-BOOK QA INSTRUCTIONS\n\nUse this package as the complete source for Book OCR Studio visual QA.\n\nRead CHATGPT-QA-INSTRUCTIONS.md first for the canonical visual-QA rules, exact importer schema, structural operation shapes, boundary actions, italics format, and stable-ID conventions. Everything in that file remains binding EXCEPT its chapter-by-chapter stop/return cadence. The Muse/Wren workflow below replaces that cadence.\n\n## MUSE WORKFLOW OVERRIDE — PROCESS THE WHOLE BOOK\n\nProcess the ENTIRE book in one continuous unattended run, sequentially one chapter at a time internally.\n\n- Start with the first chapter and continue through the final chapter/epilogue.\n- Do not skip chapters.\n- Do not parallelize chapters.\n- Do not regenerate earlier completed chapters from memory.\n- After each chapter, append that chapter to the SAME cumulative JSON and save/update that same cumulative file before moving on.\n- Continue automatically to the next chapter without waiting for a user prompt.\n- Earlier chapter records must remain byte-for-byte unchanged unless the immediately adjacent chapter boundary proves that one specific prior boundary decision must be corrected.\n- At the end, return ONE complete importer-ready cumulative JSON containing every chapter.\n\n## ABSOLUTE OUTPUT RULE\n\nThe final file MUST use this exact top-level structure:\n\n{\n  \"schema\": \"book-ocr-studio-cumulative-chapter-qa-checkpoint-v1\",\n  \"sourceMap\": {\n    \"pageCount\": 0\n  },\n  \"chapters\": []\n}\n\nSet sourceMap.pageCount to the FULL page count from typography-map.json.\n\nDo NOT:\n- invent a Muse-specific schema\n- make sourceMap a string\n- output a prose QA report instead of importer JSON\n- switch field names halfway through the book\n- use ad-hoc keys such as chapterIndex, pagesChecked, annotations, dropcaps, headerText, ocrText, correctedText, qaType, or chaptersCompleted as substitutes for canonical fields\n- change the JSON shape in later chapters\n\nEvery chapter must use the canonical chapter / structural / typography shape documented in CHATGPT-QA-INSTRUCTIONS.md.\n\n## PIXEL-BY-PIXEL COMPLETENESS\n\nInspect EVERY page in sequence and EVERY visible book-content line on each page.\n\nFor every reviewed page inspect:\n- body text against OCR\n- missing, duplicated, substituted, or hallucinated characters and words\n- opening and closing quotation marks\n- apostrophes and punctuation\n- paragraph starts and ends\n- page-boundary continuations\n- drop caps and decorative initials\n- scene-break ornaments\n- chapter headings and chapter titles/subtitles\n- POV labels when present\n- italics WORD BY WORD\n- captured headers/footers/page numbers or non-book UI\n- any missing first visual line after an ornament/drop cap\n\nDo not sample, skim, spot-check, or infer that later pages follow earlier patterns.\n\nAn empty confidentItalicSpans array means: \"I visually inspected this chapter for italics and found none.\" It must NEVER mean: \"I skipped italics.\"\n\n## STABLE-ID VALIDATION — DO NOT GUESS IDS\n\nBefore writing ANY correction, verify its IDs directly in typography-map.json.\n\nLine IDs and word/item IDs are independent sequences. NEVER infer a wordId from a lineId.\n\nExample: p0001-l0006 does NOT imply p0001-w00006.\n\nFor each replace_text operation:\n1. Find the actual affected rawOcrItems entry or entries in typography-map.json.\n2. Copy the exact existing wordId/itemId or itemIds.\n3. Copy matchText from the exact OCR item(s) being replaced.\n4. Confirm the pageId exists.\n5. Confirm replacementText is the COMPLETE screenshot-authoritative replacement for exactly those anchored OCR items.\n\nNever invent, approximate, or pattern-generate an item ID.\n\n## SILENT CONTENT-LOSS GUARD — CRITICAL\n\nPreservation of real source text is more important than aggressive cleanup.\n\nA replace_text operation REPLACES the complete anchored OCR text. Therefore replacementText must contain ALL legitimate source text belonging to those anchored item(s), not merely the fragment you are trying to fix.\n\nNEVER:\n- delete neighboring valid prose to repair a drop cap\n- replace a multi-sentence OCR item with only the corrected ending\n- use a later sentence as replacementText when the anchored OCR item contains an earlier sentence too\n- erase a paragraph beginning to insert missing text elsewhere\n- shorten an item merely because part of it appears duplicated or visually awkward\n- treat absent OCR at a scene break as permission to overwrite adjacent prose\n\nIf replacementText is shorter than matchText by more than trivial punctuation/garbage cleanup, REOPEN THE SCREENSHOT and verify the deletion character by character.\n\nIf an edit removes any whole real word, clause, sentence, or line, perform a second source-pixel verification before allowing it into the confident cumulative JSON.\n\nIf you cannot prove the deletion from the screenshot pixels, do not make it.\n\nWhen restoring text that OCR omitted, anchor to the smallest safe existing item and PRESERVE that item's existing legitimate text while adding the missing source text in the correct position.\n\n## DROPCAPS\n\nDrop caps are visual evidence, not a separate importer object.\n\nIf OCR omitted the initial character, restore it through a canonical replace_text operation anchored to the correct OCR item(s).\n\nIf OCR hallucinated garbage from the ornamental glyph, remove ONLY that garbage while preserving all real prose in the anchored item(s).\n\nAlways inspect the complete first visual line of the paragraph. Drop-cap failures may involve more than the single initial letter.\n\n## ITALICS ARE REQUIRED\n\nInspect italics visually on every page.\n\nFor each chapter, always include:\n- typography.confidentItalicSpans\n- typography.uncertainSpans\n\nUse exact canonical span/segment shapes from CHATGPT-QA-INSTRUCTIONS.md.\n\nDo not infer italics from semantics. Pixels decide.\n\nDo not extend an italic span through Roman neighbors just because they share an OCR item.\n\nIf genuinely uncertain, use uncertainSpans rather than guessing.\n\n## PAGE BOUNDARIES ARE REQUIRED\n\nReview every consecutive page boundary.\n\nFor every within-chapter boundary, add the appropriate structural.boundaryAudit decision:\n- keep_separate\n- merge_across_pages\n\nUse chapter_boundary where appropriate at chapter transitions as documented by the canonical schema.\n\nDo not decide from grammar alone. Use screenshot paragraph geometry and structure.\n\nIf merge_across_pages is required, include the corresponding canonical structural operation.\n\n## SCENE BREAKS — CURRENT SCHEMA LIMITATION\n\nVisually inspect and recognize every scene break.\n\nIf OCR captured an ornamental token that can safely be replaced with the canonical semantic marker \"* * *\", a source-proven replace_text correction may be used ONLY when doing so does not delete any legitimate prose.\n\nIf the screenshot contains a scene break but OCR contains NO safe token/item to replace, DO NOT overwrite neighboring prose, invent an unsupported operation type, or fabricate an item ID. Preserve the prose and report that unanchored scene break to the user after the JSON is complete. Book OCR Studio currently needs a future canonical insertion operation for that case.\n\n## CHAPTER METADATA\n\nUse the screenshot pixels and user-supplied book-specific notes.\n\nRecord:\n- exact source-faithful chapter heading/title in chapter.heading\n- exact POV label in chapter.pov when the source establishes one\n- exact reviewedPageRange\n- boundaryCheck.nextPageId when a following page exists\n\nDo not normalize quirky source wording.\n\n## SAVE DISCIPLINE\n\nAfter EACH chapter:\n1. validate that chapter\n2. append it to the existing cumulative JSON\n3. save/update the same cumulative file\n4. verify all earlier chapters are still present\n5. continue automatically\n\nDo not produce 28 unrelated chapter files. Do not rebuild the whole JSON from memory at the end.\n\n## FINAL SELF-CHECK BEFORE RETURNING\n\nDo not return the final file until ALL checks pass:\n\n- schema is exactly book-ocr-studio-cumulative-chapter-qa-checkpoint-v1\n- sourceMap is an object and pageCount equals the full package page count\n- chapters contains every chapter/epilogue exactly once and in order\n- every chapter uses the same canonical shape\n- every reviewed page is covered with no unintended gaps\n- every pageId exists in typography-map.json\n- every itemId/itemIds entry exists in typography-map.json\n- every matchText corresponds to the actual anchored OCR item(s)\n- operation IDs are unique\n- italic span IDs are unique\n- every chapter contains both italic arrays, even when empty\n- italics were actually visually inspected\n- every within-chapter page boundary was visually inspected\n- no correction silently deletes source-valid prose\n- no unsupported operation type was invented\n- no Muse-only/ad-hoc schema keys replaced canonical fields\n- no earlier chapter disappeared during cumulative saves\n\nThe final work product is ONE importer-ready cumulative JSON. Keep user-facing commentary brief and separately mention any visually confirmed scene break that could not be safely encoded because no OCR item existed to anchor it.\n";

      zip.file("typography-map.json", JSON.stringify(payload, null, 2));
      zip.file("QA-INSTRUCTIONS.md", qaRouterInstructions);
      zip.file("CHATGPT-QA-INSTRUCTIONS.md", chatgptQaInstructions);
      zip.file("MUSE-QA-INSTRUCTIONS.md", museQaInstructions);
      zip.file("README.txt", "Book OCR Studio visual QA package\n\nREAD QA-INSTRUCTIONS.md FIRST, then use the engine-specific instruction file: CHATGPT-QA-INSTRUCTIONS.md or MUSE-QA-INSTRUCTIONS.md. Open typography-map.json for OCR geometry and stable IDs. Original screenshots are in images/. No Auto Italics Scan guesses or training labels are included.\n");
  ...
  function qaCanonicalMapped(text) {
    const source = String(text || "");
    let normalized = "";
    const map = [];
  ...
  function qaCanonicalText(text) {
    return qaCanonicalMapped(text).normalized.trim();
  }

  function qaCanonicalOccurrences(text, needle) {
    const mapped = qaCanonicalMapped(text);
    const target = qaCanonicalText(needle);
    if (!target) return [];

    const hits = [];
    let from = 0;
    while (from <= mapped.normalized.length) {
      const at = mapped.normalized.indexOf(target, from);
      if (at < 0) break;
      const last = at + target.length - 1;
      const start = mapped.map[at] ?? 0;
      const end = (mapped.map[last] ?? start) + 1;
      hits.push({ start, end, text:mapped.source.slice(start, end), canonical:true });
      from = at + Math.max(1, target.length);
    }
    return hits;
  }
  ...
  function qaReplaceCanonicalOnce(text, matchText, replacementText) {
    const hits = qaCanonicalOccurrences(text, matchText);
  ...
  function qaItemAnchorRange(page, op) {
    ...
    const firstIndex = Math.min(...targetIndices);
    const lastIndex = Math.max(...targetIndices);
    const rawChunk = rawItems
      .slice(firstIndex, lastIndex + 1)
      .map(item => String(item?.text || item?.value || item?.label || ""))
      .join("\n");

    const candidates = [
      ...qaCanonicalOccurrences(page?.text || "", rawChunk).map(hit => ({...hit, mode:"raw-item-anchor"}))
    ];

    const correctedChunk = qaReplaceCanonicalOnce(rawChunk, op?.matchText, op?.replacementText);
    if (correctedChunk && correctedChunk !== rawChunk) {
      for (const hit of qaCanonicalOccurrences(page?.text || "", correctedChunk)) {
        candidates.push({...hit, mode:"corrected-item-anchor"});
      }
    }
  ...
  function qaCandidateSet(text, needle) {
    const exact = qaExactOccurrences(text, needle).map(hit => ({...hit, mode:"exact"}));
    if (exact.length) return exact;
    const flexible = qaFlexibleOccurrences(text, needle).map(hit => ({...hit, mode:"flexible"}));
  ...
  function qaLocateReplacement(page, op) {
    ...
    if (sourceCandidates.length > 1 || replacementCandidates.length > 1) {
      return {
        ...
        replacementCount:replacementCandidates.length,
        anchorFound:!!anchor
      };
    }
  ...
  function qaReplaceText(page, op) {
    ...
    if (located.error) {
      return {
        ...
        sourceCount:located.sourceCount,
        replacementCount:located.replacementCount,
        anchorFound:located.anchorFound
      };
    }
